...
@section('content')
    ...
    @foreach($providers as $provider)
        @if ($provider->courses()->where('state', 'started')->count()!=0)
            <div class="row">
                <div class="col">
                    <h5 style="font-weight: 300;"><a target="_blank" href="{{$provider->site}}">{{$provider->name}}</a></h5>
                </div>
            </div>
            <div class="row" style="margin-top: 15px;">
                        ...
                        @if ($course->state == 'started')
                            ...
                            <div class="card"
                                ...
                                <div class="card-body" style="background-color: rgba(255,255,255,0.9);">
                                    ...
                                    @endif
                                    <h5 style="margin-top: 15px; font-weight: 300;" class="card-title">{{$course->name}}</h5>
                                ...
                                <div class="card-footer" style="background-color: rgba(245,245,245,1);">
                                    @if ($user->role=='teacher' || $course->students->contains($user))
                                        <a href="{{url('insider/courses/'.$course->id)}}" class="btn btn-success btn-sm"><i
                                                    class="icon ion-ios-list-outline"></i>&nbsp;Страница
                                            курса</a>
                                    @else
                                        <a href="#" class="btn btn-success btn-sm disabled" role="button" aria-disabled="true">Вы
                                            не
                                            записаны</a>
                                    @endif
